...
|   01 | `01_generate_tcga_median_os_labels.py`      | Generate TCGA OS, event, and Risk_Label files using the empirical TCGA median OS cutoff. |
...
|   05 | `05_run_ablation_comparators.py`            | Run ablation and comparator models and post lock CGGA evaluation.                        |
|   06 | `06_plot_figure3_ablation_performance.py`   | Generate the ablation and external validation performance figure.                        |
...
The core Python dependencies required by the final numbered workflow are listed in `requirements_freeze.txt`.
...
* Local GMT pathway collections for GSEA and ORA analyses
...
This repository does not redistribute patient level TCGA or CGGA derived matrices, clinical files, prediction tables, fitted model objects, or local intermediate artifacts. Users must obtain authorized input data from the original data sources and follow the relevant data use terms for TCGA, CGGA, STRING, MSigDB, Reactome, and related resources.

The `.gitignore` file excludes local data directories, patient level matrices, clinical and label files, prediction tables, model objects, GMT files, STRING files, logs, and temporary files from version control.
